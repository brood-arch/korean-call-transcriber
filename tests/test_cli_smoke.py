...
def test_pyproject_exposes_console_scripts():
    ...
    assert scripts["kct-transcribe"] == "kct.transcribe.batch_transcribe:main"
    assert scripts["kct-extract"] == "kct.extract.extract_all:main"
    assert scripts["kct-health"] == "kct.pipeline.health_check:main"
    assert scripts["kct-gap"] == "kct.queue.gap_analyzer:main"
    assert scripts["kct-sync-obsidian"] == "kct.sync.sync_obsidian:main"
