"""Tests for kct.todo.persistent_store — Jaccard dedup, same-source merge, state I/O."""
...
def _isolate_state(tmp_path, monkeypatch):
    ...
    import kct.todo.persistent_store as ps
...
def test_jaccard_identical_strings():
    from kct.todo.persistent_store import _jaccard_similarity
...
def test_is_fuzzy_match_identical():
    from kct.todo.persistent_store import _is_fuzzy_match
...
def test_dedup_same_source_merges_identical():
    from kct.todo.persistent_store import _dedup_same_source
    todos = [
...
def test_completed_set_loaded_from_file(tmp_path):
    from kct.todo.persistent_store import _load_completed_titles
...
def test_is_completed_fuzzy(tmp_path):
    from kct.todo.persistent_store import _is_completed_fuzzy, _load_completed_titles
    completed = tmp_path / "completed_todos.json"
    completed.write_text(json.dumps([
...
def test_load_store_empty(tmp_path):
    from kct.todo.persistent_store import load_store
    store = load_store()
...
def test_save_and_load_roundtrip(tmp_path):
    from kct.todo.persistent_store import load_store, save_store
    store = load_store()
...
def test_save_atomic_write(tmp_path):
    from kct.todo.persistent_store import save_store
...
def test_merge_todos_adds_new(monkeypatch):
    from kct.todo import persistent_store as ps
...
def test_merge_todos_skips_completed(tmp_path):
    from kct.todo.persistent_store import load_store, merge_todos
...
def test_get_active_excludes_completed(tmp_path):
    from kct.todo.persistent_store import get_active, load_store
    completed = tmp_path / "completed_todos.json"
    completed.write_text(json.dumps([
        {"title": "done", "source": ""}
    ], ensure_ascii=False), encoding="utf-8")
...
def test_todo_key():
    from kct.todo.persistent_store import todo_key
...
def test_sync_completed(tmp_path):
    from kct.todo.persistent_store import sync_completed_to_file
    store = {"todos": {
