...
New code should prefer ``kct.config`` for runtime paths and ``kct.config.WORKSPACE``
for workspace resolution.  This module retains WSL/Windows path conversion helpers
and re-exports legacy path constants so that existing ``from kct.pipeline.paths
...
# Re-export canonical path constants from kct.config so that existing imports
# continue to resolve correctly.
from kct.config import (  # noqa: F401 – re-exports
    AUDIO_DIR,
    LOG_DIR,
    OBSIDIAN_VAULT,
    STATE_DIR,
